...
var BlockUserSchema = require('../../schema/admin/blockUser');
var AbuseUserReportSchema = require('../../schema/admin/abuseUserReport');
...
addVideoCallModel: async function (data, callback) 
{
    ...
    if(userDetails)
    {
        ...
        for (let index = 0; index < userDetails.length; index++) {
            ...
            var blockUserDetails = await BlockUserSchema.findOne({fromUserId:currUserId})
	        console.log('blockUserDetails---->',blockUserDetails);
                ...
                if(data.userType == '1')
                {
                    UserSchema.update(
                        {_id: data._id},
                        {
                            ...
                            $set:{
                                ...
                                "expert.city":data.googleAddress?(JSON.parse(data.googleAddress)).city:userData.personal.city,
                                "expert.country":data.googleAddress?(JSON.parse(data.googleAddress)).country:userData.personal.country,
                    ...
                    .then(r =>{
                        ...
                        UserSchema.update(
                            {_id: data._id},
                            {
                                ...
                                $set:{
                                    ...
                                    "personal.fullname": data.fullname?data.fullname:userData.expert.fullname ,
                                    "personal.email": data.email?String(data.email).toLowerCase():userData.expert.email,
                                    "personal.phone": data.phone?data.phone:userData.expert.phone,
                                    "personal.gender": data.gender?data.gender:userData.expert.gender,
                                    "personal.age": data.age?data.age:userData.expert.age,
                                    "personal.location": data.location?data.location:userData.expert.location,
                        ...
                        .then(async r =>{
                               ...
                               let expertdetail = await UserSchema.findOne({_id: data._id})
                                .populate('expert.specialization')
                                let out ={}
                                if(expertdetail !==null)
                                {
                                    let fieldArray = []
                                    //console.log('user---------->',user.expert.specialization)
                                    if(expertdetail.expert.specialization.length > 0)
                                    {
                                        for (let index = 0; index < expertdetail.expert.specialization.length; index++)
                                         {
                                            let fieldDetail = await FieldSchema.findOne({_id: expertdetail.expert.specialization[index].fieldId})
                                    ...
                                    out = {_id: data._id , ...expertdetail.expert ,fieldname:fieldArray}

                                    //{_id: data._id , ...expertdetail.expert,fieldname:fieldArray }
                                    callback({
                                        success: true,
                                        STATUSCODE: 2000,
                                        message: "Success",
...
//#endregion Field
                                ...
                                'personal.email': data.email.toLowerCase(),
                                   ...
                                   let userdata = await UserSchema.findOne({
                                    ...
                                    callback({
                                        ...
                                        response: {_id: userdata._id, ...userdata.personal}
                                    });
                                }                                               
                    }
                });
    ...
    socialRegister: (data, callback) => {
        ...
        console.log('data.socialLogin---',JSON.parse(data.socialLogin));
        
        UserSchema.findOne({
           // "personal.email": String(data.email).toLowerCase()
           ...(data.email !== '' ? { "personal.email": data.email } : {}),
           ...(data.socialLogin ? { "personal.socialLogin.socialId": JSON.parse(data.socialLogin).socialId } : {})
        }, async (err, user) => {
            if (err) {
                ...
                if (user) {
                    ...
                    let udetail = await UserSchema.findOne({
                        ...(data.email !== '' ? { "personal.email": data.email } : {}),
                        ...(data.socialLogin ? { "personal.socialLogin.socialId": JSON.parse(data.socialLogin).socialId  } : {})
                    })
                    .populate('expert.specialization')
                    ...
                    let fieldArray = []
                    console.log('user---------->',udetail.expert.specialization)
                    if(udetail.expert.specialization.length > 0)
                    {
                        for (let index = 0; index < udetail.expert.specialization.length; index++)
                            {
                            let fieldDetail = await FieldSchema.findOne({_id: udetail.expert.specialization[index].fieldId})
                    ...
                    callback({
                        ...
                        response: {
                            ...
                            profileImage: data.profileImage,
                            settingGenderPersonal: user.personal.settingGender,
                            settingTalkLocationPersonal: user.personal.settingTalkLocation,
                            settingSpecializationPersonal: user.personal.settingSpecialization,
                            settingConsultingFeesPersonal: user.personal.settingConsultingFees,
                            settingStartagePersonal: user.personal.settingStartage,
                            settingEndagePersonal: user.personal.settingEndage,
                            location:user.personal.location,
                            "userTypePersonal": user.personal.userType ,
                            "permissionPersonal": user.personal.permission,
                            ...
                            settingGenderExpert: user.expert.settingGender,
                            settingTalkLocationExpert: user.expert.settingTalkLocation,
                            settingSpecializationExpert: user.expert.settingSpecialization,
                            settingConsultingFeesExpert: user.expert.settingConsultingFees,
                            settingStartageExpert: user.expert.settingStartage,
                            settingEndageExpert: user.expert.settingEndage,
                            "userTypeExpert": user.expert.userType,
                            "permissionExpert": user.expert.permission,
                            "blockStatus": user.expert.blockStatus
    ...
    getAbuseReportModel: async function (data, callback) {

        let query = {} 

        if(data.fromUserId)
        {
            query['fromUserId'] = data.fromUserId
        }

        AbuseUserReportSchema.find(query)
            .populate('fromUserId') 
            .populate('toUserId') 
            .then(res =>{
                callback({
    ...
    addAbuseReportModel: async function (data, callback) {
        
        if(data){

           let duplicateBlock = await AbuseUserReportSchema.findOne(
                {
                ...
                if(duplicateBlock !== null)
                {
                    callback({
                        success: true,
                        STATUSCODE: 4200,
                        message: "User Abuse Report Already Submitted.",
                        response: "User Abuse Report Already Submitted."
                    });

                }else{

                    new AbuseUserReportSchema(data).save(async function (err, result) {
                        if (err) {
                            callback({
                                success: false,
                                STATUSCODE: 5005,
                            ...
                            callback({
                                success: true,
                                STATUSCODE: 2000,
                                message: "User Abuse Report Submitted Successfully.",
                                response: "User Abuse Report Submitted Successfully"
                            });
                        }
                    });
            }
    ...
    getBlockUserModel: async function (data, callback) {
        let query = {} 
    ...
    addBlockUserModel: async function (data, callback) {
        
        if(data){
            let duplicateBlock = await BlockUserSchema.findOne(
                {
                 "fromUserId" : data.fromUserId,
                 "toUserId"   : data.toUserId
                })
                console.log('blovk----',duplicateBlock);
                ...
                if(duplicateBlock !== null)
                {
                    callback({
                        success: true,
                        STATUSCODE: 4200,
                        message: "User Block Already Submitted.",
                        response: "User Block Already Submitted."
                    });

                }else{
                new BlockUserSchema(data).save(async function (err, result) {
                    if (err) {
                        callback({
                            success: false,
                            STATUSCODE: 5005,
    ...
    editBlockUserModel: async function (data, callback) {
        ...
        if(data){
            let duplicateBlock = await BlockUserSchema.findOne(
                ...
                if(duplicateBlock !== null)
                {
                    ...
                    await BlockUserSchema.update(
                        {fromUserId:data.fromUserId, toUserId: data.toUserId},
                        {
                            $set: {blockUserStatus:data.blockUserStatus}
                        })
                    ...
                    callback({
                        ...
                        response: "User Block Status updated Successfully."
                    });
