...
userLogin: function (userData, callback) {
    ...
    if (userData.email && userData.password) {
        ...
        UserSchema.findOne({ email: String(userData.email).toLowerCase() })
            .select('settingGender settingTalkLocation settingField settingSpecialization settingConsultingFees settingStartage settingEndage password fullname permission authtoken blockStatus userType')
            .lean(true)
            .then(function (loginRes) {
                console.log("loginRes",loginRes);
...
//#region User
    ...
    // addUserService: async function (data, fileData, callback) {
    //     let uploadedFiles = [];
    //     let folderPath = config.uploadUserPath;
    //     let count = 0;
    //     console.log('----fileData----',fileData)
    //     if (fileData) {
    //         if(fileData.profileImage.length) {
    //             console.log('----fileData 1----')

    //             for (let img of fileData.profileImage) {
    ...
    //                         })
    //                     } else {
    //                         let pushFile = config.userPath + fileName;
    //                         //uploadedFiles.push(pushFile);
    //                         count++;
    //                         if (count === fileData.profileImage.length) {
    //                             data.profileImage = pushFile;
    //                             CommonModel.addUserModel(data, function (result) {
    ...
    //             /** For single profileImage chosen */
    //             let pic = fileData.profileImage;

    //             let ext = pic.name.slice(pic.name.lastIndexOf('.'));
    //             let fileName = Date.now() + ext;
    //             console.log('----fileData 2 pic----',fileName)

    //            await pic.mv(folderPath + fileName, (err) => {
    //                 if (err) {
    //                     callback({
    //                         "success": false,
    //                         "STATUSCODE": 5005,
    ...
    //                     let pushFile = config.userPath + fileName;
    //                    // uploadedFiles.push(pushFile);
    //                    console.log('----fileData 2-1 pushFile----',pushFile)
    ...
    //         uploadedFiles[0] = config.userPath + 'no-img.jpg';
    ...
    //             {
    //                 // // console.log('----fileData 1----')
    //                 // console.log('----------------------------------------have fileData 11----',fileData)
    ...
    //                 uploadedFiles[0] = config.userPath + 'no-img.jpg';
    //                 // 'https://nodeserver.brainiuminfotech.com:3110/uploads/user/no-img.jpg'
    //                 data.profileImage = uploadedFiles;
    //                 CommonModel.addUserModel(data, function (result) {
    //                     callback(result)
    //                 });
    ...
    addUserService: async function (data, fileData, callback) {
        ...
        if(fileData)
        {
            ...
            if (Object.keys(fileData).length) 
            {
                ...
                await pic.mv(folderPath + fileName, async (err) => 
                {
                        if (err) {
                            callback({
                                "success": false,
                        ...
                        console.log('---======================================-fileData 2-1 pushFile----',pushFile)
                            ...
                            CommonModel.addUserModel(data, async function (result) {   
                            
                            console.log('result---->',result.response._id)
    ...
    editUserService: function (data,fileData, callback) {
        ...
        console.log('fileData-->',fileData)
        // if (!Object.keys(fileData).length || fileData === null ) 
            ...
            pic.mv(folderpath + fileName, function (err) {
                if (err) {
                    ...
                    if(data.profileImage == (config.userPath + fileName) )
                    {
                      ...
                      console.log('step 2 -------------------------   data.profileImage -------------->',  data.profileImage )
...
//                     uploadedFiles = data.profileImage;
//                 }
//                 for (let img of fileData.profileImage) {
//                     let ext = img.name.slice(img.name.lastIndexOf('.'));
//                     let fileName = Date.now() + ext;
...
//         } else {
//             console.log('edit user 3')

//             if (data.profileImage) {
//                 uploadedFiles = data.profileImage.length>0 ? data.profileImage[0]:data.profileImage;
//                 data.profileImage = uploadedFiles;
//             } else {
//                 uploadedFiles[0] = config.userPath + 'no-img.jpg';
//                 data.profileImage = uploadedFiles;
